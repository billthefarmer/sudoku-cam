...
// Next array represents the result of the training of the zones
// intensity features. Greater the number, the zone is darker.  It is
// automatically generated in the function Train() The function
// Train() also creates the file zones.bmp, which shows how this array
// looks.
...
// Next array represents the result of the training of the
// (width*100/height) ratios features.  It is generated in the
// function Train() Note how all the values are similar (~64), except
// the first (46), which is number 1.

const int OCRDigit::m_ratios[9] =
{46, 64, 64, 66, 64, 66, 60, 66, 66};
...
//////////////////////////////////////////////////////////////////////////
// Function:	Classify()
// Purpose:	Classifies the passed blob. It compares the blob to m_zon[]
//		and m_ratios[]. Less difference is, more probable that this
//		is the value. It is simple 'k-nearest neighbours' method
//		 with k=1.
// Parameters:	blob - input blob to be OCR-ed
//		lb, rt - pixel coordinates of the input blob
//		cell - cell to update. Updated are members cell->OCR[]
//		and cell->value.
// Returns:
/////////////////////////////////////////////////////////////////////////

void OCRDigit::Classify(const BYTE* blob, const POINT& lb,
                        const POINT& rt, SudSolverCell* cell)
{
    int test[5][5] = {0};
    ...
    // next loop compares the test[] with all nine m_zons[] by summing
    // up the differences between test[] and m_zons[].  the goal is to
    // detect the lowest difference.
    ...
    if (result != 1)
    {
        if ((width * 100) / height < 37)	// if blob ratio
            // (height:width) is less than
            // 37% this is digit 1 for
            // sure. Because is too slim
            // to be anything else.
        {
            result = 1;
            cell->OCR[1] = 0;
    ...
    {
        if ((width * 100) / height > 53)	// if blob ratio
            // (height:width) is more than
            // 53% this can't be digit 1
            // for sure. It is too thick
            // for 1.
        {
            cell->OCR[1] = INT_MAX;
...
//		It dumps the result on debug output window, ready for
//		copy-paste to variables m_zon and m_ratios.
//		Also creates zone.bmp file to see how the "ideal" numbers
//		looks like.
// Parameters:
// Returns:
/////////////////////////////////////////////////////////////////////////
...
void OCRDigit::Train(CString filename, int digit,
                     int (*zon)[5][5], RatioWH *ratio)
{
    int sum, count;

    // MUST be grayscale image
    HBITMAP hBitmap =
        (HBITMAP)::LoadImage(NULL, filename,
                             IMAGE_BITMAP, 0, 0,
