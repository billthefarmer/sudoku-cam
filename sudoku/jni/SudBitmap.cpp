...
SudBitmap::SudBitmap(DWORD bitsResolution, LPVOID buf,
                     CSize sz, const DisplaySud* display)
{
...
//		Uses "Mean Adaptive Thresholding"
//		method. http://homepages.inf.ed.ac.uk/rbf/HIPR2/adpthrsh.htm

//		Optimized algorithm with the use of integral image.
...
void SudBitmap::Monochrome()
{
    int x, y, yOffset;
    unsigned int sum;
    unsigned int* pIntegral = new unsigned int[height * width];
    // integral image (sums of the original image)
    BYTE mean, byte;
    static const BYTE C = 2;	// it is better 1 in some cases
    static const int RECT = 5;	// TODO dynamically adapt value from 3
    // to 8 based on image size
    ...
    case 24:
    {
        ...
        // fill integral image. First step is to copy orig.image
        // -> integral image

        for (x = width * height - 1; x >= 0; x--)
        {
            pix = buf[x];
            pIntegral[x] = (((pix.red * 77) +
                             (pix.green * 150) +
                             (pix.blue * 28)) >> 8) & 255;
        ...
        for (y = RECT + 1; y < height - RECT; y++)
        {
            ...
            for (x = RECT + 1; x < width - RECT; x++)
            {
                ...
                sum = pIntegral[(y + RECT) * width + x + RECT] -
                      pIntegral[(y - RECT - 1) * width + x + RECT] -
                      pIntegral[(y + RECT) * width + x - RECT - 1] +
                      pIntegral[(y - RECT - 1) * width + x - RECT - 1];
                mean = (BYTE)(sum / count);
                // calculate mean value of RECTxRECT neighbours pixels

                pix = buf[yOffset + x];
                byte = (BYTE)((((pix.red * 77) + (pix.green * 150) +
                                (pix.blue * 28)) >> 8) & 255);
                // 100% Intesity = 30% Red + 59% Green + 11% Blue

                if (byte < mean - C)	// original pixel intensity
                    // above or below the local
                    // threshold?
    ...
    case 32:
    {
        ...
        for (y = RECT + 1; y < height - RECT; y++)
        {
            ...
            for (x = RECT + 1; x < width - RECT; x++)
            {
                // sum the surounding pixels RECTxRECT around
                // x,y. By using integral image this goes fast.
                sum = pIntegral[(y + RECT) * width + x + RECT] -
                      pIntegral[(y - RECT - 1) * width + x + RECT] -
                      pIntegral[(y + RECT) * width + x - RECT - 1] +
                      pIntegral[(y - RECT - 1) * width + x - RECT - 1];
                mean = (BYTE)(sum / count);	// calculate mean
                // value of RECTxRECT
                // neighbours pixels

                pix = buf[yOffset + x];
                byte = (((GetRValue(pix) * 77) +
                         (GetGValue(pix) * 150) +
                         (GetBValue(pix) * 28)) >> 8) & 255;
                // 100% Intesity = 30% Red + 59% Green + 11% Blue
                if (byte < mean - C)	// original pixel intensity
                    // above or below local
                    // threshold?
                    m_pMono[y * width + x] = 0;	// black
                // else
                //	m_pMono[y*width + x] = 255;	// white
...
// Parameters:	pIntegral - pointer to the integral image to build.
//		Initially must be filled with the copy of original image.
/////////////////////////////////////////////////////////////////////////

void SudBitmap::SumIntegralImage(unsigned int* pIntegral)
{
    int x, y;
    unsigned int *pIntegralA, *pIntegralB,
             *pIntegralC, *pIntegralD;	// use pointers for speed
...
// Purpose:	Detects the image rotation angle by detecting the strongest
//		line around the centre of the image.
//		http://en.wikipedia.org/wiki/Hough_transform
// Parameters:	pStrongestLine - pointer to the resulting detected
//		strongest line.
...
BOOL SudBitmap::HoughTransformCenter(Bin* pStrongestLine)
{
    ...
    // fill accumulator from image, we will sweep the small portion at
    // the the image center (not entire image) for better performance

    for (y = 3 * strip; y < 4 * strip; y++)
        // image center strip, which is 1/7 of the height
    {
        yOffset = y * width;
        for (x = width / 3; x < width - width / 3; x += 3)
            // 1/3 of the width; skip every 3 pixels
        {
            if (display->rotation)
            {
                COLORREF color = GetPixel(x, y);
                SetPixel(x, y, RGB(GetRValue(color), GetGValue(color),
                                   GetBValue(color) / 3));
            }
            ...
            if (m_pMono[yOffset + x] == 0)	// if black pixel
            {
                ...
                // looking for horizontal lines around 90(+-30)
                // degrees
            }
        }
    }
    ...
    // Display the accumulator (of the hough transformation) to the
    // screen Nice to visualize what the accumulator is. Uncomment
    // next block to visualize it.
    ...
    {
        {	byte = (BYTE)(accumulator.bins[rho * accumulator.thetas +
    				       theta]) * 5;
    	y = (int)(rho * factor);
    ...
    if (display->rotation)
    {
        ...
        {
            int y = (int)((x * cos((180 - pStrongestLine->theta) * DEG2RAD) +
                           pStrongestLine->rho - m_rhos) /
                          sin((180 - pStrongestLine->theta) * DEG2RAD));
...
//		http://en.wikipedia.org/wiki/Hough_transform
// Parameters:	inputLine - reference line (rho value) from which to
//		detetect the strongest line.
//		offset - parallel distance from inputLine where to search
//		for the strongest line. Could be negative or positive,
//		depending on the direction from the inputLine.
...
BOOL SudBitmap::HoughTransform(Line inputLine, int offset, int loLimit,
                               ...
                               Line* pStrongestLine)
{
        ...
        for (rho = start; rho < end; rho += 1)	// try 5 for speed
        {
            for (y = loLimit; y < hiLimit; y += 2)	// try 4 for speed
            {
                ...
                if (x > 0 && x < width && y > 0 && y < height)
                {
                    ...
                    {
                        accumulator.Add(y, x, inputLine.theta - thetaOffset,
                                        inputLine.theta + thetaOffset);
                        // sweep +- degrees
                    }
                }
            }
        ...
        for (rho = start; rho < end; rho += 1)	// try 5
        {
            for (x = loLimit; x < hiLimit; x += 2)	// try 4
            {
                ...
                if (x > 0 && x < width && y > 0 && y < height)
                {
                    ...
                    if (byte == 0)		// if black pixel
                    {
                        accumulator.Add(x, y, inputLine.theta - thetaOffset,
                                        inputLine.theta + thetaOffset);
                        // sweep +- degrees
                    }
                }
    ...
    {
        yOffset = rho * accumulator.thetas;
        for (x = inputLine.theta - thetaOffset;
                x <= inputLine.theta + thetaOffset; x++)
        {
...
//		Step 3. Detects the intersection points of grid lines.
//			Fills them in m_grid, which is the goal of this method.
...
BOOL SudBitmap::DetectRect(int theta)
{
    ...
    for (rho = start; rho < end; rho += 3)
        // todo: try with 5 for performance
    {
        // sweep down->up with parallel lines and count white
        // pixels. Our goal is to find longest uninterrupted white
        // strip line. Strip consists of rectangles 3x3 pixels.
        int whiteLen = 0;
    ...
    for (rho = start; rho < end; rho += 3)	// todo: try 5 for performance
    {
        ...
        int Ydown = (int)(((rho - start) * cos(bottom.theta * DEG2RAD) +
                    ...
                    {
                        {
                            ...
                            if (display->whiteCandidates)
                                SetPixel(x, yy, RGB(100, 100, 50));
                            // display tracing to screen
                        }
                    }
                }
                if (count / 3 > sum)	// white
    ...
    ///////////////////////////////////////////////////
    // detect the black lines closest to the white line. First the
    // left and right lines. Then based on the distance between them,
    // detect the third and the sixth line, which sometimes are bolded
    // compared to other lines. To be image skew proof.  The goal is
    // to find m_grid[][] points.
    Line hor[10], ver[10];
    Line temp;
...
//////////////////////////////////////////////////////////////////////////
// Function:    GetRect()
// Purpose:     Return the four corners of the detected rectangle.
//
//////////////////////////////////////////////////////////////////////////
...
void SudBitmap::DrawRect(const Line& left, const Line& top, const Line& right,
...
// Purpose:	Gets a single pixel intesity from the input image.
//		Converts the color to gray. Blue part is darkest,
//		red is lightest.
// Parameters:	x, y - pixel coordinates
...
// Purpose:	Shows a number on the output image. Numbers are bitmaps
//		stored in m_font.
// Parameters:	yBottom, xLeft - coordinates of the grid cell. Not pixel
//		coordinates.
//		color - color to draw with
//		numberToDisplay - number to display
// Returns:
/////////////////////////////////////////////////////////////////////////
void SudBitmap::DisplayCellNumber(int yBottom, int xLeft, COLORREF color,
                                  int numberToDisplay)
{
...
//////////////////////////////////////////////////////////////////////////
// Function:	OCR()
// Purpose:	Performs OCR on all sudoku cells. If needed, fixes the OCR
//		values based on sudoku rules.
// Parameters:
        ...
        for (y = 8; y >= 0; y--)
            for (x = 0; x < 9; x++)
                if (m_cells.cell[y][x].value != 0)
                    DisplayCellNumber(y, x, RGB(10, 50, 0),
				      m_cells.cell[y][x].value);
...
//////////////////////////////////////////////////////////////////////////
// Function:	GetPuzzle()
// Purpose:	Fills in the array provided
// Parameters:	puzzle - int array to fill in
//
//////////////////////////////////////////////////////////////////////////

void SudBitmap::GetPuzzle(int puzzle[9][9])
{
    for (int y = 0; y < 9; y++)
	for (int x = 0; x < 9; x++)
	    puzzle[y][x] = m_cells.cell[y][x].value;
}
...
BOOL SudBitmap::Solve()
{
    ...
    if (m_cells.SolveMe() == TRUE)
    {
        ...
        memcpy(m_solution, tempResult, sizeof(tempResult));
        // copy it to m_solution
...
//////////////////////////////////////////////////////////////////////////
// Function:	OCRCell()
// Purpose:	Recognize a cell. Cell value could be empty (which is 0)
//		or number 1-9.
// Parameters:	yBottom, xLeft - coordinates of the grid cell. Not pixel
//		coordinates.
// Returns:
...
void SudBitmap::OCRCell(int yBottom, int xLeft)
{
    ...
    int cellWidth  = (rt.x - 1) - (lt.x + 2);
    int cellHeight = (lt.y - 1) - (lb.y + 2);
    POINT reslb, resrt;		// enclosing corners of the blob - it
    // is the goal of this function to
    // calculate it
    BYTE blob[100][100];	// bitmap of the cell - UNROTATED.
    // TODO: make sure we don't get cells
    // bigger than 100x100.
    ...
    if (sum < (cellWidth * cellHeight) / 31)
        // threshold between empty cell and cell with a digit
        // inside. Obtained empirically.
    {
    ...
    // hor line scan sweep bottom part (first 1/3 of the
    // height). Start from the 1/3 of the height and go down to the
    // bottom
    bIsFirstWhite = FALSE;
    ...
    for (int y = cellHeight / 3; y >= 0; y--)
    {
        ...
        {
            if (blob[y][x] == 0)	// if at least one black pixel
                // found in that line, go to
                // next line
            {
                reslb.y = y;
                bIsFirstWhite = FALSE;
    ...
    // uncomment next block to save wrongly recognized blobs on disk
    // (current directory) in BMP format type your initial sudoku
    // values in gold[][] array
...
/////////////////////////////////////////////////////////////////////////
/*
void SudBitmap::SaveBitmapForTraining(const BYTE* blob, const POINT& lb,
                                      const POINT& rt, int yBottom,
                                      int xLeft, int value)
{
