...
{
    int value;		// The cell recornition value itself. Could be
			// inital or calculated.
    int OCR[10];	// OCR probability for each digit 1-9. Smaller
			// value means bigger probability for that
			// digit value.
    BOOL isCandidate[10];	// List of possible values for this
				// cell. Ignored for initally set
				// cells.
};
...
public:
    ...
    SudSolverCell cell[9][9];	// sudoku grid values. If the cell is
				// initally empty the value is 0.
private:
