...
void SudResultVoter::Reset()
{
    ...
    for (int i = 0; i < MAXRESULTS; i++)
        m_solutions[i].vote = m_solutions[i].vote / (MAXCOUNTER / 10);
    // don't reset the existing values to zero. Just make them lower.
}
...
void SudResultVoter::GetBestSolution(Solution *result)
{
    ...
    // if solution doesn't exist in m_solutions insert it. Overwrite
    // the worst solution.
    if (bFound == FALSE)
    {
